...
digraph prd_writer {
    ...
    update_source [label="Step 3.5: Update source\nrequirements file"];
    generate [label="Phase 4: Generate complete PRD"];
    ...
    gaps_found -> update_source [label="no, all complete"];
...
- Track which dimensions have been sufficiently covered. When ALL dimensions are at "sufficient" status, proceed to Step 3.5.
- **Track changes for source document update:** For every user-confirmed answer, internally classify it as:
  - **Addition（新增）:** Information not in the original file at all
  - **Clarification（澄清）:** Vague content made specific
  - **Correction（纠正）:** User explicitly corrected original content

  These classifications will be used in Step 3.5 to update the source requirements file.

### Step 3.5: Update Source Document

**Goal:** Reflect all confirmed gap-filling information back into the original requirements description file, so it stays in sync with the refined understanding.

<HARD-GATE>
Do NOT skip this step. The requirements description file is the single source of truth for upstream decisions. If it's outdated, all downstream documents (PRD, technical design, etc.) will be built on stale foundations.
</HARD-GATE>

**Actions:**
- Compile all user-confirmed additions, clarifications, and corrections from Step 3
- Re-read the original requirements description file
- Update the corresponding sections in-place:
  - Add new user scenarios discovered during gap-filling to the "User Scenarios" section
  - Refine user persona with more specific details confirmed by the user
  - Add clarified business rules, constraints, and edge cases
  - Update risk section with newly identified risks
  - Incorporate any scope changes the user explicitly confirmed to include
- Preserve the original document structure and format — do NOT restructure or rewrite unaffected sections
- After updating, inform the user with a change summary:
  ```
  我已根据我们讨论的内容更新了需求描述文件 [文件路径]，主要变更如下：
...
**Rules:**
- Only write information that has been explicitly confirmed by the user during Step 3
- Do NOT delete or overwrite existing content unless the user explicitly corrected it
- Do NOT add your own assumptions or inferences — only confirmed facts
- If the source file was generated by `requirement-discovery`, follow its document structure and conventions
- Proceed to Step 4 after the user confirms the updates (or immediately if no changes were needed)
...
When the user confirms a scope change should be included, classify it as an **Addition** and ensure it is written back to the source requirements file in Step 3.5. The requirements description file should always reflect the latest agreed-upon scope, so downstream documents stay aligned.
